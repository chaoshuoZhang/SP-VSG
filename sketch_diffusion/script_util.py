import argparse
import inspect
from .distribution import AxisAlignedConvGaussian
from . import gaussian_diffusion as gd
from . import gaussian_diffusion_noise as gdn
from . import gaussian_diffusion_acc as gda
...
def create_model_and_diffusion_noise(
    ...
    diffusion = create_gaussian_diffusion_noise(
        steps=diffusion_steps,
        learn_sigma=learn_sigma,
        sigma_small=sigma_small,
        noise_schedule=noise_schedule,
...
def create_model_and_diffusion_acc(
    ...
    diffusion = create_gaussian_diffusion_acc(
        steps=diffusion_steps,
        learn_sigma=learn_sigma,
        sigma_small=sigma_small,
        noise_schedule=noise_schedule,
...
def create_gaussian_diffusion_noise(
    ...
    if use_kl:
        loss_type = gdn.LossType.RESCALED_KL
    elif rescale_learned_sigmas:
        loss_type = gdn.LossType.RESCALED_MSE
    else:
        loss_type = gdn.LossType.MSE
    if not timestep_respacing:
        timestep_respacing = [steps]
    return SpacedDiffusion_noise(
        use_timesteps=space_timesteps(steps, timestep_respacing),
        betas=betas,
        model_mean_type=(
            gdn.ModelMeanType.EPSILON if not predict_xstart else gdn.ModelMeanType.START_X
        ),
        model_var_type=(
            (
                gdn.ModelVarType.FIXED_LARGE_NOISE
                if not sigma_small
                else gdn.ModelVarType.FIXED_SMALL
                # gdn.ModelVarType.FIXED_SMALL
            )
            if not learn_sigma
            else gdn.ModelVarType.LEARNED_RANGE
        ),
        loss_type=loss_type,
        rescale_timesteps=rescale_timesteps,
    )
...
def create_gaussian_diffusion_acc(
    ...
    return SpacedDiffusion_acc(
        ...
        model_var_type=(
            (
                gda.ModelVarType.FIXED_LARGE_ACC
                if not sigma_small
                else gda.ModelVarType.FIXED_SMALL
                # gda.ModelVarType.FIXED_SMALL
            )
            if not learn_sigma
