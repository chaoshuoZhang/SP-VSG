...
class DPM_Solver:
    def __init__(
        ...
        self.tensor_parm = torch.from_numpy(tensor_parm).float()

    def slerp(self, tensor_max, tensor_min=None, mu=0.5):
        '''
        使用PyTorch实现球面线性插值（Slerp）

        参数:
        tensor_max -- 形状为 batch*2N*3 的张量
        tensor_min -- 形状为 batch*N*3 的张量
        mu -- 插值参数 保留max

        返回:
        形状为 batch*2N*3 的插值结果张量
        '''
        # 将numpy数组转换为torch张量
        # tensor_max = torch.from_numpy(tensor_max).float()
        # tensor_min = torch.from_numpy(tensor_min).float()

        # tensor_max = self.model_fn(x, t)
        if tensor_min is None:
            tensor_min = self.tensor_parm.to(tensor_max.device)
        # # 计算点积，针对最后一个维度
        # dot_product = torch.sum(tensor_min * tensor_max[:, :tensor_min.shape[1]], dim=-1)
        #
        # # 限制点积的值范围为[-1, 1]
        ...
        #
        # # 避免除零错误，对于sin(theta)接近零的情况，使用一个小的正数替代
        # sin_theta = torch.where(sin_theta < 1e-6, torch.full_like(sin_theta, 1e-6), sin_theta)
        #
        # # 计算插值的系数
        # coeff1 = torch.sin((1.0 - mu) * theta) / sin_theta
        # coeff2 = torch.sin(mu * theta) / sin_theta
        #
        # # 扩展系数维度以支持广播
        ...
        # coeff2 = mu
        # # 计算插值向量
        # result = coeff1 * tensor_min + coeff2 * tensor_max[:, :tensor_min.shape[1]]
        #
        # # 为了匹配tensor_max的形状，我们需要在tensor_min中补零
        # # 创建一个与tensor_max相同形状的零张量
        # zero_padding = 1 * tensor_max
        # # 将插值结果放在零张量的相应位置
        # zero_padding[:, :tensor_min.shape[1]] = result
                ...
                for step in tqdm(range(order, steps + 1)):
                    ...
                    if step < steps:
                        ...
                        if step == self.slerp_step:
                            model_prev_list[0] = self.slerp(model_prev_list[0])
                    else:
