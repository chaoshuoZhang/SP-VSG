...
class GaussianDiffusion:
    ...
    def mse_cross_entropy(self, pen_state, target_pen_state, mse):
        ...
        max_values, _ = th.max(pen_state, dim=1)
        pen_state = pen_state - max_values.unsqueeze(1)
        Soft_pen_state = Softmax(pen_state)
        max_pen_state = th.gather(Soft_pen_state, 1, target_pen_state.view(-1, 1))
        loss_list = -th.log(max_pen_state)
        loss = mse * loss_list

        return loss.mean(dim=0)

    def cal_se_entropy(self, pen_state, target_pen_state, min_loss=0.0001):
        Softmax = th.nn.Softmax(dim=1)
        max_values, _ = th.max(pen_state, dim=1)
        pen_state = pen_state - max_values.unsqueeze(1)
        Soft_pen_state = Softmax(pen_state)
        max_pen_state = th.gather(Soft_pen_state, 1, target_pen_state.view(-1, 1))
        loss_list = -th.log(max_pen_state)
        condition = loss_list > 0.001
        result = th.where(condition, loss_list, th.tensor(min_loss))
        mse = th.sum(result ** 2) / len(result)
        return mse

    def geometric_mean_cross_entropy(self, pen_state, target_pen_state, min_loss=0.0001):
        Softmax = th.nn.Softmax(dim=1)
        max_values, _ = th.max(pen_state, dim=1)
        pen_state = pen_state - max_values.unsqueeze(1)
        Soft_pen_state = Softmax(pen_state)
        ...
        condition = loss_list > 0.001
        result = th.where(condition, loss_list, th.tensor(min_loss))
        product = result ** (1.0 / loss_list.size(0))
        geometric_mean = th.prod(product)
        # loss = loss_list.mean()
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output, pen_state = model(x, self._scale_timesteps(t), **model_kwargs)
        model_output_noise = int(th.sum(model_output).cpu())
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_variance, model_log_variance = {
                ModelVarType.FIXED_LARGE_ORIGIN: (
        ...
        return {
            ...
            "pen_state": pen_state,
            "model_output_noise": model_output_noise,
        }
    ...
    def ddim_sample(
        ...
        if flat:
            t = torch.tensor([0] * x.shape[0], device=t.device)
        out = self.p_mean_variance(
            model,
            x,
            t,
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output, pen_state = model(x_t, self._scale_timesteps(t), **model_kwargs)
