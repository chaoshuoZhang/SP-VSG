...
our_category_list = ["sketchrnn_apple.npz"]
...
def scale_sketch(sketch, size=(448, 448)):
    ...
    if h >= w:
        sketch_normalize = sketch / np.array([[h, h, 1]])
    else:
        sketch_normalize = sketch / np.array([[w, w, 1]])
    sketch_rescale = sketch_normalize * np.array([[size[0], size[1], 1]])
    return sketch_rescale.astype("int16")
...
if __name__ == '__main__':
    ...
    for each_cate in our_category_list:
        ...
        if not os.path.exists(f"D:/2023_9/SketchKnitter-pro/dataset/real_data/{each_cate.replace('.npz', '')}"):
            os.mkdir(f"D:/2023_9/SketchKnitter-pro/dataset/real_data/{cat_name}")
        npz_load = np.load(f"D:/2023_9/SketchKnitter-pro/dataset/{each_cate}", allow_pickle=True, encoding="latin1")[
            "test"]
        for index, each_sketch in enumerate(npz_load):
            sketch_cv = draw_three(each_sketch, img_size=40)
            all.append(sketch_cv)
            # cv2.imwrite(f"D:/2023_9/SketchKnitter-pro/dataset/real_data/{cat_name}/{index}.jpg", sketch_cv)
            print(f"{cat_name}, {index}")
    all = np.array(all)
    np.savez(os.path.join('D:/2023_9/SketchKnitter-pro/dataset/real_data/', 'result_apple_40_transpose.npz'), all)
